...
思衡 V6 是一个默认在本地验收、可经单独授权部署为教师演示站的实验性访谈 Demo。用户只面对一位访谈官“澄澄”；模型依据完整逐字稿决定从哪里开始、怎样承接以及是否继续。六维构念只作为模型内化的观察视角，**不再驱动实时题库、阶段、coverage 或轮次编排**。
...
- 首问和每一次追问默认由 `natural_interviewer_v6.0.5` 生成；`v6.1.1` 是必须显式配置才会启用的候选，完整保留 `v6.0.4` 与 `v6.0.3` 作为回退。候选版从真实、具体、需要判断或权衡的事件开始，默认沿同一事件柔性深入，在跑题时于一轮内拉回，并把正常探查控制为 1—2 句话、最多一个主要问题。每个会话绑定开场 trace 记录的 Prompt 版本，部署切换或回滚只影响新会话。模型以 `finish/enough_understanding|natural_closure` 表达自然结束意图；服务端对 v6.1.1 或已接受新版“用户确认结束”说明的会话映射为 `suggest_finish` 并保持可继续，旧说明下的旧版会话保留原冻结语义。只有用户明确要求结束时才直接返回 `finish/user_requested`。服务端仍不提供候选题库、目标维度、coverage、阶段命令或测量轮次上下限。
...
- 用户可主动“结束并生成报告”或“退出不生成报告”；接受新版开始页说明的会话中，访谈官认为内容已足够理解或自然收束时只会提出可接受或忽略的结束建议，不会替用户自动交卷。刷新后可从持久化会话恢复，同一 `client_turn_id` 不得重复写入回答。

## Git 与来源

活动基线分支为 `system/v6-natural-interview-demo`。V6 从 V5 的本地工作区快照建立：来源分支 `system/v5-real-issue-demo`、来源提交 `e7ae0246cfa7c9e3cb1e43c8938c740d4cb945ea`。完整的来源边界、排除项与 SHA-256 聚合指纹见 [来源快照清单](artifacts/SOURCE_SNAPSHOT_MANIFEST.md)。
