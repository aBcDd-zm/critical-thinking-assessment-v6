...
思衡 V6 是一个默认在本地验收、可经单独授权部署为教师演示站的实验性访谈 Demo。用户只面对一位访谈官“澄澄”；访谈模型依据完整逐字稿自然承接，独立后台任务在每轮后先切分并校验证据归属，再更新六维证据快照。六维构念**不驱动实时题库、阶段、coverage 或固定轮次编排**；只有至少 8 个已保存用户回答且六维证据全部充分时，系统才主动显示完整报告入口。
...
- 首问和每一次追问默认由 `natural_interviewer_v6.2.1` 生成：从真实、具体、需要判断或权衡的事件开始，沿同一决策主线柔性深入；遇到 AI、论文或他人材料与本人判断混合时先中性澄清来源，再回到原决策。`natural_interviewer_v6.2.3` 是只能显式选择的语气候选：完整继承 v6.2.1 的决策主线、来源归属和结束保护，只细化明确脆弱经历下的有据温和承接、尚无具体事件时的选择权邀请，并要求正常探查恰好保留一个清晰问题。内部 `navigation` 只记录真实 user span 锚点、焦点和主线关系，不显示给参与者，也不恢复六维导航、阶段或题库。每个会话绑定开场 trace 的 Prompt 版本；配置切换只影响新会话。
- 访谈轮次使用关闭思考、512 tokens、首次 30 秒且全链路 80 秒封顶的配置。新会话默认绑定 `V6.2.1 + enforce`：每轮后先完成独立证据归属，再仅用服务端判定为 eligible 的 span ID 自动评分。归属失败、格式错误或陈旧资产不得回退到未过滤评分。切换前已创建的 shadow/disabled 会话继续使用开场 trace 冻结的原合同，不中途换版。
- 首个回答只要非空即可提交；从第二个回答起，普通回答仍需 20 个可见字符，完整的“不知道／不清楚／没想好”类表达例外。该例外不改变作答计数、证据语义或自然收束规则。
- 8 个已保存用户回答是主动收束的最低保护线，不是固定结束点；八轮后证据不足仍继续。40 次用户回答只是技术防失控上限；到达时前端才显示中性说明和报告入口，不把它渲染为目标进度、证据充分性或作答质量判断。
- 每份 enforce 快照都包含完整六维结果及其 `attribution_span_id`、理由、置信度、优势和改进重点；数字分、优势与优先关注点只能引用同一检查中经服务端校验的 eligible span。用户结束时，服务端校验检查 ID、逐字稿和资产指纹后将同一快照提升为正式评分与报告，报告阶段不再调用模型或重复评分。旧报告保持可读，旧证据只标为 `legacy_unclassified`，绝不反推为本人观点。
...
- [V6.2.1 证据归属与决策主线](docs/V6_2_1_EVIDENCE_ATTRIBUTION.md)
- [V6.2.3 自然承接语气候选](docs/V6_2_3_NATURAL_WARMTH_CANDIDATE.md)
...
V6.2.1 开发分支为 `system/v621-evidence-attribution`，直接叠加在 `system/v612-consistent-closure` 的 `7a44abf` 上；V6.2 Draft PR 保持独立。V6 最初从 V5 的本地工作区快照建立；完整来源边界见 [来源快照清单](artifacts/SOURCE_SNAPSHOT_MANIFEST.md)。
