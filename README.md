...
- 访谈轮次使用关闭思考、512 tokens、25 秒总预算的低延迟配置。每轮后的后台链路以 `EVIDENCE_ATTRIBUTION_MODE=disabled|shadow|enforce` 发布：开发默认 `shadow` 记录归属与 span 评分对照但不改变旧结果；验收通过后，`enforce` 才允许明确绑定 V6.2.1 的新会话仅用服务端判定为 eligible 的 span 自动评分。归属失败、格式错误或陈旧资产不得回退到未过滤评分。
