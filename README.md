...
- 首问和每一次追问均由 `natural_interviewer_v6.0.3` 生成。服务端不提供候选题库、目标维度、coverage、阶段命令或测量轮次上下限；访谈官以简短共情承接而非机械复述，并以开放问题自然探查一到两层仍关键的不确定性或反例后收束；明确表达“不知道”等不确定时，改用低压力的开放式引导。
