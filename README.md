...
- 首问和每一次追问默认由 `natural_interviewer_v6.2.1` 生成：从真实、具体、需要判断或权衡的事件开始，沿同一决策主线柔性深入；遇到 AI、论文或他人材料与本人判断混合时先中性澄清来源，再回到原决策。内部 `navigation` 只记录真实 user span 锚点、焦点和主线关系，不显示给参与者，也不恢复六维导航、阶段或题库。每个会话绑定开场 trace 的 Prompt 版本；`v6.2.0` 及更早版本原文和回滚路径完整保留。
