...
- 首问和每一次追问默认由 `natural_interviewer_v6.0.5` 生成；`v6.1.1` 是必须显式配置才会启用的候选，完整保留 `v6.0.4` 与 `v6.0.3` 作为回退。候选版从真实、具体、需要判断或权衡的事件开始，默认沿同一事件柔性深入，并在跑题时于一轮内拉回。每个会话绑定开场 trace 记录的 Prompt 版本，部署切换或回滚只影响新会话。模型以 `finish/enough_understanding|natural_closure` 表达自然结束意图；服务端对 v6.1.1 或已接受新版“用户确认结束”说明的会话映射为 `suggest_finish` 并保持可继续，旧说明下的旧版会话保留原冻结语义。只有用户明确要求结束时才直接返回 `finish/user_requested`。服务端仍不提供候选题库、目标维度、coverage、阶段命令或测量轮次上下限。
