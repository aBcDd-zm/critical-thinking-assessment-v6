...
本交接包供队友审查、整合导师后续建议并部署。它冻结当前 V6.1 产品实现，
不把尚未完成的真实 UAT 表述为已通过，也不将本轮自动化测试等同于心理测量效度验证。

本轮不再修改以下内容：

- `natural_interviewer_v6.1.0` Prompt 与自然访谈逻辑；
...
- `POST /sessions/{uuid}/finalize` 是唯一的幂等终评入口；访谈逐字稿冻结后才允许评分。
...
- 豆包 TTS 属于另一条可选能力；当前生产 Compose 默认 `TTS_MODE=disabled`，
  本交接不要求启用或验收。

## 4. 2026-08-05 零 API 自动化验收

以下命令均未调用真实模型、未读取真实被试数据：
