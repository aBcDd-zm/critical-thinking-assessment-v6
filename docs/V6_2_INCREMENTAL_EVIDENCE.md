...
`v6.1.1`、`v6.0.5`、`v6.0.4` 和 `v6.0.3` 的 Prompt 原文与哈希保持不变。会话使用 `natural_opening` trace 绑定开场版本，因此环境变量回滚只影响新会话。V6.2 数据库迁移只扩展既有 `evidence_readiness_checks`，不修改 Rubric、报告公开结构或 API 路径。
