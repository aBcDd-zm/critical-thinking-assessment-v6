...
- `NATURAL_INTERVIEWER_PROMPT_VERSION` 默认为 `v6.0.5`；`v6.1.0` 仅作为显式启用的轻引导候选，`v6.0.4` 与 `v6.0.3` 完整保留为回滚版本。V6.1.0 只增加真实事件锚定、同一事件柔性主线和极简拉回，不引入题库、阶段、维度轮询或第二模型。
