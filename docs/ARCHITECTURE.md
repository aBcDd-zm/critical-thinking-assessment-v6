...
flowchart LR
    ...
    C --> S["V6 会话编排\n状态、幂等、安全、审计"]
    S --> I["natural_interviewer_v6.0.2\n完整逐字稿 → 一次自然回应"]
    I --> S
...
服务端可以拦截、记录或拒绝以下情况：未同意、少于 20 个可见字符的回答、当前会话不接收回答、相同幂等键载荷不同、高风险内容、空/无效 JSON、模型泄露内部 Prompt/评分、明显有害输出以及 40 次技术上限。普通风格问题只写入质量标记：服务端不把自然问法替换成题库文本，也不替模型指定下一题。
